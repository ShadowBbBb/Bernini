...
- 7b51738 grad-ckpt 改 `use_reentrant=False`（HF 4.57.3 默认 reentrant 不接受 **kwargs，WanTransformerBlock 的 cu_seqlens_* 等 kwarg 被拒）
- 本次：`clip_grad_norm_(model.parameters(), ...)`（原传 `model` 对象不可迭代，DDP 不是 iterable of params）
