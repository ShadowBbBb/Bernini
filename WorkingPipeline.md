...
- **RULE 5: todo.md 维护。** 每次给服务器下达执行命令时，**重写** `todo.md`（覆盖旧内容，不留历史），只放本次要执行的那一组命令。服务器侧 `git pull` 后可直接 `cat todo.md` 复制执行。
- **RULE 6: history.md 维护。** 每次代码/配置变更提交后，向 `history.md` **追加**一条摘要（一句话 + commit 短号），不记详细内容；历史按时间顺序累积，不删旧条目。